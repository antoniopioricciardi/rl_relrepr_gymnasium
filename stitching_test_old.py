...
def init_carracing_env(car_mode="standard", background_color="green", image_path=None, cust_seed=0, render_md="rgb_array"):
    if car_mode == "slow":
    ...
    env = CarRacing(continuous=False, background=background_color, image_path=image_path, render_mode='human')
...
def init_env(env_id, background_color="green", image_path=None, cust_seed=0, render_md="human"):
    if env_id.startswith("CarRacing-v2"):
        # separate car mode from env_id
        car_mode = env_id.split('-')[-1]
        nv = init_carracing_env(car_mode=car_mode, background_color=background_color, image_path=image_path, cust_seed=cust_seed, render_md=render_md)
...
def test_get_trajectory(env):
    ...
    obs = env.reset()
    track = env.track
    trajectory[0] = (env.car.hull.position[0], env.car.hull.position[1], env.car.hull.angle)
    ...
    while not done_testing:
        ...
        if score > max_ep_score:
            max_ep_score = score
        obs = next_obs
        trajectory[i] = (env.car.hull.position[0], env.car.hull.position[1], env.car.hull.angle)
        done_testing = done
...
if single_test:
    ...
    if car_mode == "slow":
        ...
        from car_racing import CarRacing
    env = CarRacing(continuous=False, background=args.background_color, image_path=image_path, render_mode='human')
    env = make_env_atari(
        env, seed=cust_seed, rgb=True, stack=4, no_op=0, action_repeat=0,
        max_frames=False, episodic_life=False, clip_reward=True, check_fire=False, idx=0, capture_video=False, run_name='test'
        )()
    ...
    encoder1 = load_encoder_from_path(
        path1_enc, encoder_instance, is_relative=False, is_pretrained=False, anchors_alpha=None, encoder_eval=True, device=device
        )    
    encoder2, policy2, agent2 = load_model_from_path(
        path2_enc, path2_pol, env.action_space.n,
    ...
    if stitching_md == "translate":
        ...
        print("\n##############################################\n")
        print(
            f"fitting translation layer between encoder and policy models spaces..."# {model_color_1} and {model_color_2} spaces..."
        )
...
if single_test:
    ...
    print(f"Score: {score} - Max Episode Score: {max_ep_score} - Episode Length: {i} steps")
    # save track to csv
    ...
    trajectory_df = pd.DataFrame.from_dict(trajectory, orient='index', columns=['x', 'y', 'angle'])

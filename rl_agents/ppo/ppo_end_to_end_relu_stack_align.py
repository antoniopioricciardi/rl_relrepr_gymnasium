...
class FeatureExtractor(nn.Module):
    def __init__(self, use_relative=False, pretrained=False, obs_anchors=None, obs_anchors_filename=None, anchors_alpha=0.99, device='cpu'):#, anchors_std=None):
        super().__init__()
    ...
    def forward(self, x):
        num_stack = x.shape[1]
        x = x.view(x.shape[0] * x.shape[1], x.shape[2], x.shape[3], x.shape[4]) # (batch_size * stack, 3, 84, 84)
        ...
        hidden = hidden.view(hidden.shape[0], num_stack * hidden.shape[2]) # (batch_size, num_stack * 3136) -> (batch_size, 12544)
        return hidden
...
class Agent(nn.Module):
    def __init__(self, feature_extractor: FeatureExtractor, policy: Policy, translation=None):
        super().__init__()
    ...
    def get_action_and_value(self, x, action=None):
        if self.translation is None:
            ...
            hid = self.translation(hid).vectors.view(1, 12544)#['target'].view(1, 12544)

...
if __name__ == "__main__":
    ...
    if args.background == "plain":
        ...
        env = NaturalEnvWrapper(args.env_id, imgsource="color", color=args.background,
                                resource_files=None, render_mode='rgb_array', zoom=4)
        eval_env = NaturalEnvWrapper(args.env_id, imgsource="color", color=args.background,
                                resource_files=None, render_mode='rgb_array', zoom=4)
    ...
    envs = gym.vector.AsyncVectorEnv([ 
        make_env_atari(
            env, seed=args.seed, stack=args.stack_n, no_op=30, action_repeat=args.stack_n,
            max_frames=False, episodic_life=True, clip_reward=True, check_fire=True, idx=i, capture_video=False, run_name=run_name
            )
    ...
    eval_envs = gym.vector.AsyncVectorEnv([
        make_env_atari(
            eval_env, seed=args.seed, stack=args.stack_n, no_op=30, action_repeat=args.stack_n,
            max_frames=False, episodic_life=True, clip_reward=True, check_fire=True, idx=i, capture_video=False, run_name=eval_run_name
            )

...
def motion_feature_extraction(frames):
    ...
    Eigens = np.zeros(
        (motion_vectors.shape[0], motion_vectors.shape[1], motion_vectors.shape[2], 2),
        dtype=np.float32,
    )
...
def compute_niqe_features(frames):
    ...
    assert (M >= blocksizerow * 2) & (
        N >= blocksizecol * 2
    ), "Video too small for NIQE extraction"
...
def temporal_dc_variation_feature_extraction(frames):
    ...
    dct_motion_comp_diff = np.zeros(
        (motion_vectors.shape[0], motion_vectors.shape[1], motion_vectors.shape[2]),
        dtype=np.float32,
    )
    for i in range(motion_vectors.shape[0]):
        for y in range(motion_vectors.shape[1]):
            for x in range(motion_vectors.shape[2]):
                ...
                patchI = frames[
                    i,
                    y * mblock + motion_vectors[i, y, x, 0] : (y + 1) * mblock
                    + motion_vectors[i, y, x, 0],
                    x * mblock + motion_vectors[i, y, x, 1] : (x + 1) * mblock
                    + motion_vectors[i, y, x, 1],
                    0,
                ].astype(np.float32)
                diff = patchP - patchI
                t = scipy.fftpack.dct(
                    scipy.fftpack.dct(diff, axis=1, norm="ortho"), axis=0, norm="ortho"
                )
...
def NSS_spectral_ratios_feature_extraction(frames):
    ...
    dct_diff5x5 = np.zeros(
        (
            frames.shape[0] - 1,
            np.int(frames.shape[1] / mblock),
            np.int(frames.shape[2] / mblock),
            mblock**2,
        ),
        dtype=np.float32,
    )
    for i in range(dct_diff5x5.shape[0]):
        for y in range(dct_diff5x5.shape[1]):
            for x in range(dct_diff5x5.shape[2]):
                diff = frames[
                    i + 1, y * mblock : (y + 1) * mblock, x * mblock : (x + 1) * mblock
                ].astype(np.float32) - frames[
                    i, y * mblock : (y + 1) * mblock, x * mblock : (x + 1) * mblock
                ].astype(np.float32)
                t = scipy.fftpack.dct(
                    scipy.fftpack.dct(diff, axis=1, norm="ortho"), axis=0, norm="ortho"
                )
    ...
    mf_gamma5x5 = freq_bands[
        :, np.int((mblock**2 - 1) / 3) + 1 : 2 * np.int((mblock**2 - 1) / 3) + 1
    ]
...
def videobliinds_features(videoData):
    ...
    assert C == 1, (
        "videobliinds called with video having %d channels. Please supply only the luminance channel."
        % (C,)
    )

...
def compute_msssim(frame1, frame2, method="product"):
    ...
    for i in range(level):
        mssim_array, ssim_map_array, mcs_array, cs_map_array = ssim_full(
            im1, im2, K_1=K_1, K_2=K_2, avg_window=avg_window
        )
...
def msssim(referenceVideoData, distortedVideoData, method="product"):
    ...
    assert C == 1, (
        "MS-SSIM called with videos containing %d channels. Please supply only the luminance channel"
        % (C,)
    )
    assert (M >= 176) & (N >= 176), (
        "You supplied a resolution of %dx%d. MS-SSIM can only be used with videos large enough having multiple scales. Please use only with resolutions >= 176x176."
        % (M, N)
    )

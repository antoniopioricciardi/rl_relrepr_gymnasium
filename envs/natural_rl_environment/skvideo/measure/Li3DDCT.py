...
def Li3DDCT_features(videoData):
    ...
    assert C == 1, (
        "called with video having %d channels. Please supply only the luminance channel."
        % (C,)
    )
    ...
    for idx, frameidx in enumerate(frameSlices):
        ...
        for i in range(63):
            ...
            hist, bins = np.histogram(
                np.abs(block3dDCT[:, i]), bins=Nbins, range=(0, 2 ** (16 - 1) - 1)
            )
        ...
        for j in range(Nbins):
            nonzeros = hist3d[j, :] > 0
            Energy2feats[idx, nonzeros] += -(
                hist3d[j, nonzeros] * np.log2(hist3d[j, nonzeros])
            )

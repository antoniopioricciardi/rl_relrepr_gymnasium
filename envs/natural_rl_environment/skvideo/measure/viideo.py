    ...
    for itr in range(0, np.int(n_len + 1), np.int(step_size)):
        ...
        if len(f1_cum) > 0:
            ...
            for i in range(f1_cum.shape[1]):
                if (np.sum(np.abs(f1_cum[:, i])) != 0) & (
                    np.sum(np.abs(f2_cum[:, i])) != 0
                ):
...
def viideo_features(videoData, blocksize=(18, 18), blockoverlap=(8, 8), filterlength=7):
    ...
    assert C == 1, (
        "viideo called with video having %d channels. Please supply only the luminance channel."
        % (C,)
    )
    ...
    for k in range(np.int(T / 2)):
        ...
        for itr in range(0, 2):
            mscn, _, mu = compute_image_mscn_transform(
                diff, avg_window=hf, extend_mode="nearest"
            )
            ...
            # pad arrays
            mscn = np.pad(
                mscn, ((top_pad, bot_pad), (left_pad, right_pad)), mode="constant"
            )
            h = np.pad(h, ((top_pad, bot_pad), (left_pad, right_pad)), mode="constant")
            v = np.pad(v, ((top_pad, bot_pad), (left_pad, right_pad)), mode="constant")
            d1 = np.pad(
                d1, ((top_pad, bot_pad), (left_pad, right_pad)), mode="constant"
            )
            d2 = np.pad(
                d2, ((top_pad, bot_pad), (left_pad, right_pad)), mode="constant"
            )

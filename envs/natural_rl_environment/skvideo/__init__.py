...
def scan_ffmpeg():
    ...
    try:
        ...
        version = check_output(
            [os.path.join(_FFMPEG_PATH, _FFMPEG_APPLICATION), "-version"]
        )
...
def scan_libav():
    ...
    try:
        # grab program version string
        version = check_output(
            [os.path.join(_AVCONV_PATH, _AVCONV_APPLICATION), "-version"]
        )
        # only parse the first line returned
        firstline = version.split(b"\n")[0]
...
def getFFmpegVersion():
    ...
    if _FFMPEG_MAJOR_VERSION[0] == "N":
        ...
        return "%s.%s.%s" % (
            ...
            _FFMPEG_MINOR_VERSION,
            _FFMPEG_PATCH_VERSION,
        )
...
def setFFmpegPath(path):
    ...
    if os.path.isfile(
        os.path.join(_FFMPEG_PATH, _FFMPEG_APPLICATION)
    ) and os.path.isfile(os.path.join(_FFMPEG_PATH, _FFPROBE_APPLICATION)):
        _HAS_FFMPEG = 1
    else:
...
def setLibAVPath(path):
    ...
    # check to see if the executables actually exist on these paths
    if os.path.isfile(
        os.path.join(_AVCONV_PATH, _AVCONV_APPLICATION)
    ) and os.path.isfile(os.path.join(_AVCONV_PATH, _AVPROBE_APPLICATION)):
        _HAS_AVCONV = 1
    else:

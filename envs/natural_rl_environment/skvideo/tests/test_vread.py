...
def _rawhelper1(backend):
    # reading first time
    bunnyMP4VideoData1 = skvideo.io.vread(
        skvideo.datasets.bigbuckbunny(), num_frames=1, backend=backend
    )
    ...
    skvideo.io.vwrite(
        "bunnyMP4VideoData_vwrite_" + backend + ".yuv",
        bunnyMP4VideoData1,
        backend=backend,
    )

    bunnyYUVVideoData1 = skvideo.io.vread(
        "bunnyMP4VideoData_vwrite_" + backend + ".yuv",
        width=1280,
        height=720,
        num_frames=1,
    ...
    skvideo.io.vwrite(
        "bunnyYUVVideoData_vwrite.yuv", bunnyYUVVideoData1, backend=backend
    )
    bunnyYUVVideoData2 = skvideo.io.vread(
        "bunnyYUVVideoData_vwrite.yuv",
        width=1280,
        height=720,
        num_frames=1,
        backend=backend,
    ...
    bunnyMP4VideoData2 = skvideo.io.vread(
        skvideo.datasets.bigbuckbunny(), num_frames=1, backend=backend
    )
    ...
    assert t == 0, (
        "Possible mutable default error in vread. MSE=%f between consecutive reads."
        % (t,)
    )
    ...
    assert t < error_threshold, (
        "Unacceptable precision loss (mse=%f) performing vwrite (mp4 data) -> vread (raw data)."
        % (t,)
    )
    ...
    assert t < error_threshold, (
        "Unacceptable precision loss (mse=%f) performing vwrite (raw data) -> vread (raw data)."
        % (t,)
    )
...
def _rawhelper2(backend):
    ...
    skvideo.io.vwrite(
        "bunnyMP4VideoData_vwrite.yuv",
        bunnyMP4VideoData1,
        inputdict=pipingDict.copy(),
        backend=backend,
    )
    ...
    t = np.mean((bunnyMP4VideoData1 - bunnyYUVVideoData1) ** 2)
    assert t == 0, (
        "Precision loss (mse=%f) performing vwrite (mp4 data) -> vread (raw data)."
        % (t,)
    )

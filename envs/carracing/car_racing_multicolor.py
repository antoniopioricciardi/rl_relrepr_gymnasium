...
class CarRacing(gym.Env, EzPickle):
    ...
    * `continuous=True` converts the environment to use discrete action space.
    ...
    def _init_colors(self):
        if self.domain_randomize:
            ...
            if self.background == "red":
                self.bg_color = np.array([204, 102, 102])
                self.grass_color = np.array([230, 102, 102])
            if self.background == "blue":
                self.bg_color = np.array([102, 102, 204])
                self.grass_color = np.array([102, 102, 230])
            if self.background == "violet":
                self.bg_color = np.array([204, 102, 204])
                self.grass_color = np.array([230, 102, 230])
            if self.background == "pink":
                self.bg_color = np.array([230, 102, 153])
                self.grass_color = np.array([255, 102, 178])
    ...
    def _render_road(self, zoom, translation, angle):
        ...
        if self.background in self.background_list:
            self._draw_colored_polygon(
                self.surf, field, self.bg_color, zoom, translation, angle, clip=False
            )

...
class PPOTrainer_vec:
    ...
    def train(self):
        ...
        eval_freq = (self.total_timesteps // (self.num_envs * self.num_eval_eps)) * self.num_envs # (5000000 // (16 * 1000)) * 16 = 4992
        ...
        # for update in range(1, num_updates + 1):        
            ...
            for step in range(0, self.num_steps):
                ...
                if global_step % eval_freq == 0:
                    print('### EVALUATION ###')
                    self.agent.eval()
                    eval_rewards, eval_lengths, eval_avg_reward = evaluate_vec_env(agent=self.agent, num_envs=self.num_eval_envs, env=self.eval_envs,
                                                                                global_step=global_step, device=self.device,
                                                                                episode_n=episode_n, writer=self.writer, logger=self.logger)
                    self.agent.train()
                    ...
                    if eval_avg_reward >= eval_best_score:
                        ...
                        if not self.pretrained:
                            best_model_params["encoder"] = self.agent.encoder.state_dict()
                        best_model_params["policy"] = self.agent.policy.state_dict()
                    print('Current eval_avg_reward: ', eval_avg_reward, '======= eval_best_score: ', eval_best_score)
                    ...
                    if self.track:
                        ...
                        save_model(wandb=self.wandb, log_path=self.log_path, model_params_dict=best_model_params, save_wandb=self.track)
                ...
                if next_done[0]:
                    ...
                    for info in infos["final_info"]:
                        ...
                        if info is None:
                            continue
                        # logger from meta
                        with self.logger.log_and_dump_ctx(global_step, ty='train') as log:
                        ...
                        if self.use_relative:
                            ...
                            self.wandb.log({f"anchors_embeddings/{i}": self.wandb.Histogram(self.encoder.anchors[i].cpu().detach())  for i in range(10)}, step=global_step)

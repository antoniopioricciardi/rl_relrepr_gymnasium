if __name__ == "__main__":
    ...
    global_vars = vars(zeroshotrl.carl.envs)
    vars = {
    ...
    for env_name in env_names:
        defaults = pd.Series(
            getattr(eval(getattr(zeroshotrl.carl.envs, env_name).__module__), "DEFAULT_CONTEXT")
        )  # pd.Series(vars[env_name + "_defaults"])
        n_context_features.append(len(defaults))
        bounds = getattr(
            eval(getattr(zeroshotrl.carl.envs, env_name).__module__), "CONTEXT_BOUNDS"

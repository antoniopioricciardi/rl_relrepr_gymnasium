...
def init_anchors_from_obs(use_anchors, anchors_path, model):
    ...
    if use_anchors:
        ...
        obs = torch.cat([encoding.detach().cpu() for encoding in obs_set["obs"]], dim=0)
...
def get_obs_anchors(
    anchors_path,
):  # , anchors_indices_path='anchor_indices_ppo3136.txt'):
...
def get_obs_anchors_old(
    anchors_path, anchors_indices_path="anchor_indices_ppo3136.txt"
):
    obs_set = pickle.load(Path(anchors_path).open("rb"))

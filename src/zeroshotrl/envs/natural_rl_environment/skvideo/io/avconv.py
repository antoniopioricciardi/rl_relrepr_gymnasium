...
class LibAVReader(VideoReaderAbstract):
    ...
    def __init__(self, *args, **kwargs):
        assert (
            _HAS_AVCONV
        ), "Cannot find installation of libav (which comes with avprobe)."
    ...
    def _createProcess(self, inputdict, outputdict, verbosity):
        ...
        if verbosity == 0:
            cmd = (
                [_AVCONV_PATH + "/" + _AVCONV_APPLICATION, "-nostats", "-loglevel", "0"]
                + iargs
                + ["-i", self._filename]
                + oargs
                + ["-"]
            ...
            cmd = (
                [_AVCONV_PATH + "/" + _AVCONV_APPLICATION]
                + iargs
                + ["-i", self._filename]
                + oargs
                + ["-"]
...
class LibAVWriter(VideoWriterAbstract):
    ...
    def _warmStart(self, M, N, C, dtype):
        if (
            ...
            and (
                ("-pix_fmt" not in self.inputdict)
                or (self.inputdict["-pix_fmt"][0:6] == "rgba64")
            )
        ):
            raise ValueError("libAV doesnt support rgba64 formats")
        if (
            C < 3 and "-pix_fmt" not in self.inputdict
        ):  # pix_fmt gray, ya8 and their 16 bit equivalents have a bug in LibAV
            C += 2
            self._prepareData = (
                self._gray2RGB
            )  # replace prepareData methode by the gray2RGB hack method

...
class Steerable:
    ...
    def buildSFpyrlevs(self, lodft, log_rad, angle, Xrcos, Yrcos, ht):
        if ht <= 1:
            ...
            lostart = np.ceil((dims + 0.5) / 2) - np.ceil(
                (np.ceil((dims - 0.5) / 2) + 0.5) / 2
            )
    ...
    def reconSFPyrLevs(self, coeff, log_rad, Xrcos, Yrcos, angle):
        if len(coeff) == 1:
            ...
            for b in range(int(self.nbands)):
                ...
                orientdft += (
                    (np.complex(0, 1) ** (order)) * banddft * anglemask * himask
                )
    ...
    def base(self, m, n):
        ...
        xv, yv = np.meshgrid(
            (np.array(range(n)) + 1 - ctrn), (np.array(range(m)) + 1 - ctrm)
        )
    ...
    def normalize(self, coef, height, order):
        ...
        for pyr_h in xrange(height - 2):
            ...
            for cband in xrange(order):
                ...
                if pyr_h > 0:
                    child = scipy.misc.imresize(
                        coef[pyr_h][cband], 50, interp="bilinear", mode="F"
                    )
                if pyr_h + 3 < height:
                    # parent = scipy.misc.imresize(coef[pyr_h+2][cband], 2.0, interp='bilinear', mode='F')
                    parent = scipy.misc.imresize(
                        coef[pyr_h + 2][cband], 200, interp="bilinear", mode="F"
                    )
                    ...
                    cov = np.array(
                        np.hstack(
                            (
                                ...
                                sublevel.transpose(1, 2, 0)[1:-1, 1:-1, idx].reshape(
                                    (w - 2) * (h - 2), order - 1
                                ),
...
def load_sp5filters():
    ...
    return (
        lo0filt.astype(np.float32),
        hi0filt.astype(np.float32),
        lofilt.astype(np.float32),
        bfilts.astype(np.float32),
        mtx.astype(np.float32),
        harmonics.astype(np.float32),
    )
...
class SpatialSteerablePyramid:
    ...
    def extractSingleBand(
        self, inputimage, filtfile="sp1Filters", edges="symm", band=0, level=1
    ):
        inputimage = inputimage.astype(np.float32)

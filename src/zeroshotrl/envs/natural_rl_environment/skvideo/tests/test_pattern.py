...
def pattern_sinusoid(backend):
    ...
    assert floattopixel_mse < 1, (
        "Possible conversion error between floating point and raw video. MSE=%f"
        % (floattopixel_mse,)
    )
...
def pattern_noise(backend):
    ...
    # load it and resave it to check the pipeline for drift
    videoData1 = skvideo.io.vread(
        "randomNoisePattern.yuv", width=100, height=100, backend=backend
    )

    skvideo.io.vwrite("randomNoisePattern_resaved.yuv", videoData1, backend=backend)
    videoData2 = skvideo.io.vread(
        "randomNoisePattern_resaved.yuv", width=100, height=100, backend=backend
    )

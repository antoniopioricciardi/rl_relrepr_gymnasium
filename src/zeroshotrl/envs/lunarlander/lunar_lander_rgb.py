...
class LunarLanderRGB(gym.Env, EzPickle):
    r"""
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        self.color = color
        self.color_list = ['white', 'green', 'red']
        assert self.color in self.color_list, f"color must be one of {self.color_list}"
        self.color_values = {
            'white': (255, 255, 255),
            'green': (0, 255, 0),
            'red': (255, 0, 0)
    ...
    def step(self, action):
        ...
        frame = self._render("state_pixels")
        reward -= (
            m_power * 0.30
    ...
    def _render(self, mode: str):
        ...
        pygame.draw.rect(self.surf, self.color_values[self.color], self.surf.get_rect())
        ...
        if mode == "human":
            ...
            return self._create_image_array(self.surf, (VIEWPORT_W, VIEWPORT_H))
            # return np.transpose(

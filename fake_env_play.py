...
# perform evaluation
e_obs, _ = envs.reset(seed=1)
e_obs = torch.Tensor(e_obs).to(device)
e_obs2, _ = envs2.reset(seed=1)
e_obs2 = torch.Tensor(e_obs2).to(device)
...
while not_d:  # not np.all(eval_dones):
    ...
    e_next_obs, e_reward, terminated, truncated, e_info = envs.step(
        action.cpu().numpy()
    )
    _, _, _, _, _ = envs2.step(
    action.cpu().numpy()
    )
    # sss += e_reward
